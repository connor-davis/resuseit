...
let EditCollectorModal = ({
  data,
  show,
  onEdit,
  onCancel,
}: EditCollectorModalProps) => {
  let [editStep, setEditStep] = useState(1);
  ...
  useEffect(() => {
    if (data) {
      setFirstName(data.collectorFirstName);
      setLastName(data.collectorLastName);

      setEmail(data.collectorEmail);
      setPhoneNumber(data.collectorPhoneNumber);
      setIdNumber(data.collectorIdNumber);

      setAccountNumber(data.collectorAccountNumber);
      setBranchCode(data.collectorBranchCode);
      setBankName(data.collectorBankName);

      setAddressLineOne(data.collectorStreetAddress.split(',')[0].trim());
      setAddressLineTwo(data.collectorStreetAddress.split(',')[1].trim());
      setCity(data.collectorCity);
      setPostalCode(data.collectorPostalCode);
      setProvince(data.collectorProvince);
      setCountry(data.collectorCountry);
    }
  }, [data]);
          ...
          >
            <div className="inline-block w-auto p-3 my-8 overflow-hidden text-left align-middle transition-all transform bg-gray-100 dark:bg-black shadow-xl rounded-2xl border-l border-t border-r border-b border-gray-300 dark:border-gray-800">
              <div className="text-lg font-medium leading-6 text-gray-900 dark:text-white text-center mb-5">
                {editStep === 1 && 'Details'}
                {editStep === 2 && 'Contact Details'}
                {editStep === 3 && 'Bank Details'}
                {editStep === 4 && 'Location Details'}
              </div>
              <div className="mt-2">
                {editStep === 1 && (
